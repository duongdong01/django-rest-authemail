...
class SignupVerify(APIView):
    ...
    def get(self, request, format=None):
        ...
        if verified:
            ...
            return render(request,'authemail/layout_email_verified.html',content)
            # return Response(content, status=status.HTTP_200_OK)
        else:
            content = {'detail': _('Unable to verify user.')}
            return render(request,'authemail/layout_page_not_found.html')
            # return Response(content, status=status.HTTP_400_BAD_REQUEST)
            ...
            if user:
                if user.is_verified:
                    if user.is_active:
                        ...
                        return Response({"data":{'user':userData,'token': token.key},"statusCode":200,"success":True},
...
class PasswordResetVerify(APIView):
    ...
    def get(self, request, format=None):
        ...
        try:
            ...
            return render(request,'authemail/layout_forgot_password_form.html',content)
            # return Response(content, status=status.HTTP_200_OK)
    ...
    def post(self, request, format=None):
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                # return Response(content, status=status.HTTP_200_OK)
                return render(request,'authemail/layout_forgot_password_verified.html')
            except PasswordResetCode.DoesNotExist:
                content = {'detail': _('Unable to verify user.')}
                return render(request,'authemail/layout_page_not_found.html')
                # return Response(content, status=status.HTTP_400_BAD_REQUEST)
...
class PasswordChange(APIView):
    ...
    def post(self, request, format=None):
        ...
        if serializer.is_valid():
            ...
            password = serializer.data['password']
            email = user.email
            isPassword =authenticate(email=email,password=password)
            if isPassword:
                content = {'detail':
